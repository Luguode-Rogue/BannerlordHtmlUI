...
namespace BannerlordHtmlUI
{
    ...
    internal sealed class HtmlUiHost : IDisposable
    {
        ...
        private void UiThreadMain()
        {
            ...
            catch (Exception ex) { HtmlUiLogger.Error("WebView2 UI thread failed.", ex); _ready.TrySetException(ex); }
        }
        ...
        private async Task InitializeWebView2Async()
        {
            ...
            catch (Exception ex) { HtmlUiLogger.Error("WebView2 initialization failed.", ex); _ready.TrySetException(ex); }
        }
        ...
        private void FollowBannerlordWindow()
        {
            try
            {
                ...
                if (active) { _form.Bounds = new Rectangle(rect.Left, rect.Top, width, height); if (!_form.Visible) _form.Show(); }
        ...
        internal bool UnregisterContentRoot(string id){if(string.IsNullOrWhiteSpace(id)||string.Equals(id,"framework",StringComparison.OrdinalIgnoreCase))return false;if(!_contentRoots.Remove(id))return false;_contentHosts.Remove(id);HtmlUiLogger.Info("Content root unregistered: "+id);return true;}
        public void RegisterContentRoot(string id,string directory){if(string.IsNullOrWhiteSpace(id))throw new ArgumentException("Content root id is required.",nameof(id));if(string.IsNullOrWhiteSpace(directory))throw new ArgumentException("Content root directory is required.",nameof(directory));var full=Path.GetFullPath(directory);if(!Directory.Exists(full))throw new DirectoryNotFoundException(full);if(_contentRoots.TryGetValue(id,out var existing)){if(string.Equals(existing,full,StringComparison.OrdinalIgnoreCase))return;throw new InvalidOperationException("Content root id is already registered: "+id);}RunOnUiThreadSync(()=>MapContentRoot(id,full));}
        private void RunOnUiThreadSync(Action action){if(_form==null||_form.IsDisposed)throw new InvalidOperationException("HTML UI host is not ready.");if(!_form.InvokeRequired){action();return;}Exception error=null;using(var gate=new ManualResetEventSlim(false)){_form.BeginInvoke(new Action(()=>{try{action();}catch(Exception ex){error=ex;}finally{gate.Set();}}));gate.Wait();}if(error!=null)throw error;}
        private void MapContentRoot(string id,string directory){var host=id.Equals("framework",StringComparison.OrdinalIgnoreCase)?"bannerlord-htmlui.local":"bannerlord-htmlui-"+SanitizeHostPart(id)+".local";_contentRoots[id]=directory;_contentHosts[id]=host;_web.CoreWebView2.SetVirtualHostNameToFolderMapping(host,directory,CoreWebView2HostResourceAccessKind.Allow);}
        private static string SanitizeHostPart(string value){var chars=value.ToLowerInvariant().ToCharArray();for(var i=0;i<chars.Length;i++)if(!((chars[i]>='a'&&chars[i]<='z')||(chars[i]>='0'&&chars[i]<='9')||chars[i]=='-'))chars[i]='-';var result=new string(chars).Trim('-');if(result.Length==0)result="mod";return result;}
        ...
        public void Show()=>SetInputMode(HtmlUiInputMode.Passive); public void Hide()=>SetInputMode(HtmlUiInputMode.Hidden); public void CaptureInput()=>SetInputMode(HtmlUiInputMode.Captured); public void CaptureMouse()=>SetInputMode(HtmlUiInputMode.MouseCaptured); public void ReleaseInput()=>SetInputMode(HtmlUiInputMode.Passive);
        ...
        internal void DispatchToGameThread(Action action)=>GameThread.Post(action); public bool CommandExists(string name)=>Bridge!=null&&Bridge.CommandExists(name); public bool UnregisterCommand(string name)=>Bridge!=null&&Bridge.UnregisterCommand(name); public bool UnregisterRequest(string name)=>Bridge!=null&&Bridge.UnregisterRequest(name);
        public void RegisterCommand(string name,Action<JToken> handler){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterCommand(name,handler);} internal void RegisterCommand(string name,Action<JToken> handler,string ownerId){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterCommand(name,handler,ownerId);}
        public void RegisterRequest(string name,Func<JToken,Task<object>> handler){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterRequest(name,handler);} public void RegisterRequest(string name,Func<JToken,CancellationToken,Task<object>> handler){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterRequest(name,handler);} internal void RegisterRequest(string name,Func<JToken,Task<object>> handler,string ownerId){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterRequest(name,handler,ownerId);} internal void RegisterRequest(string name,Func<JToken,CancellationToken,Task<object>> handler,string ownerId){if(Bridge==null)throw new InvalidOperationException("HTML UI host is not ready.");Bridge.RegisterRequest(name,handler,ownerId);}
