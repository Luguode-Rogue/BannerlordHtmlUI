...
namespace BannerlordHtmlUI
{
    ...
    public sealed class HtmlUiHost : IDisposable
    {
        ...
        internal void Navigate(HtmlUiPage page)
        {
            if (page == null) throw new ArgumentNullException(nameof(page));
            if (_disposed) throw new ObjectDisposedException(nameof(HtmlUiHost));
            HtmlUiLogger.Info("Navigate requested: page=" + page.Id + ", root=" + page.ContentRootId + ", path=" + page.RelativePath + ", webReady=" + IsWebViewReady);
            if (!IsWebViewReady) { _pendingPage = page; HtmlUiLogger.Warn("Navigate deferred because WebView2 is not ready: " + page.Id); return; }
        ...
        private void FlushPendingPage()
        {
            ...
            var page = _pendingPage; _pendingPage = null;
        ...
        private void NavigateOnUiThread(HtmlUiPage page)
        {
            EnsureUiThread(() =>
            {
                try
                {
                    if (_web?.CoreWebView2 == null) { _pendingPage = page; HtmlUiLogger.Warn("Navigate deferred on UI thread because CoreWebView2 is null: " + page.Id); return; }
                    ...
                    HtmlUiLogger.Info("Navigating WebView2 to " + uri + ", formVisible=" + _form.Visible + ", requestedVisible=" + _requestedVisible + ", inputMode=" + _inputMode);
                    _web.Source = uri;
                }
                catch (Exception ex)
                {
        ...
        public void SetInputMode(HtmlUiInputMode requestedMode)
        {
            // MouseCaptured is deliberately compatibility-only. The old native mouse path
            // repeatedly caused focus/click regressions; do not reactivate it here.
            var mode = requestedMode == HtmlUiInputMode.MouseCaptured ? HtmlUiInputMode.Passive : requestedMode;
            _inputMode = mode;
            ...
            EnsureUiThread(() =>
            {
                ...
                else Win32.ShowWindow(_form.Handle, Win32.SW_SHOWNOACTIVATE);
            });
        }
        ...
        private void ApplyInputModeOnUiThread()
        {
            ...
            if (_inputMode == HtmlUiInputMode.Hidden) { _form.Hide(); return; }
            if (!_form.Visible) _form.Show();
            if (_inputMode == HtmlUiInputMode.Captured) { _form.Activate(); _web?.Focus(); }
            else Win32.ShowWindow(_form.Handle, Win32.SW_SHOWNOACTIVATE);
        }
        ...
        private void EnsureUiThread(Action action)
        {
            ...
            catch (ObjectDisposedException ex) { HtmlUiLogger.Warn("UI thread host was disposed before callback scheduling: " + ex.Message); }
            catch (InvalidOperationException ex) { HtmlUiLogger.Warn("UI thread callback could not be scheduled: " + ex.Message); }
        }
        ...
        public void Dispose()
        {
            ...
            try { if (_form != null && !_form.IsDisposed) _form.Invoke(new Action(() => { _web?.Dispose(); _form.Close(); })); } catch { }
        }
    }
}